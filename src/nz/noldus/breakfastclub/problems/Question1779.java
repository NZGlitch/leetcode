...
//points[
////i] = [ai, bi] represents that a point exists at (ai, bi). A point is valid 
//if it 
...
//the 
////valid points, [2,4] and [4,4] have the smallest Manhattan distance from 
//your 
    ...
    public int nearestValidPoint(int x, int y, int[][] points) {
        ...
        for (int i=0; i<points.length; i++) {
            if (points[i][0] == x || points[i][1] == y) {
                int mdist = Math.abs(x-points[i][0]) + Math.abs(y-points[i][1]);
                if (mdist < bdist) {
                    best = i;
                    bdist = mdist;
                }
